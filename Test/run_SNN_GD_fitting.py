...
for random_seed in range(start_seed, start_seed+num_seeds):
    ...
    # load_data = torch.load(IO.PATH + GLIF.__name__ + '/' + prev_timestamp + '/' + fname + IO.fname_ext)
    # snn_target = load_data['model']
    # saved_target_losses = load_data['loss']
    # snn_target = TargetModelsBestEffort.glif(random_seed, N=4)
    # model_class = GLIF
    snn_target = TargetModelsBestEffort.lif(random_seed, N=4)
    model_class = LIF
    ...
    params_model = target_parameters.copy()
    params_model['preset_weights'] = snn_target.w.data
