...
class LIF_R_ASC_soft_lower_dim(nn.Module):
    ...
    def forward(self, I_ext):
        ...
        I_tot = ((self.I_additive + self.g) / 2).matmul(self.self_recurrence_mask * W_syn)
        dv = (self.G * (self.E_L - self.v) + (I_tot + I_ext) * self.norm_R_const) / self.tau_m
        v_next = self.v + dv
